...
namespace ESourcing.Sourcing.Repositories
{
    public class BidRepository : IBidRepository
    {
        ...
        {
            FilterDefinition<Bid> filter = Builders<Bid>.Filter.Eq(x => x.Id, id);
            List<Bid> bids = await _sourcingContext.Bids.Find(filter).ToListAsync();


            bids = bids.OrderByDescending(a => a.CreatedAt).GroupBy(a => a.SellersUsername).Select(a => new Bid
            {
                AuctionId = a.FirstOrDefault().AuctionId,
                Price = a.FirstOrDefault().Price,
                CreatedAt = a.FirstOrDefault().CreatedAt,
                SellersUsername = a.FirstOrDefault().SellersUsername,
